...
int main(int argc, char* argv[]) {
	...
	// Primary Variables
	vector<double> x0(_GLB_N_, 0);
	GUESS(_GLB_N_, x0);
	double* _x0     = (double*) gpu::alloc(x0);

	vector<double> vtemp(_GLB_N_, 0);
	double* _vtemp  = (double*) gpu::alloc(vtemp);

	vector<double> vtempl(range, 0);
	double* _vtempl = (double*) gpu::alloc(vtempl);

	vector<double> x1(_GLB_N_, 0);
	...
	// Block Schimatics
	dim3 nm_tpb (16, 16);
	dim3 nm_blocks(_GLB_N_ / nm_tpb.x + 1, range);
