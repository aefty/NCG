namespace cuda {
   ...
   __global__ void bulkGrad( long int N ,  double EPS, double* space, double* grad) {
      ...
      if (i < N) {
         space[i * N + i] -= EPS;
         FUNCTION(N, &space[i * N], &cache[threadIdx.x]);

         space[i * N + i] += 2.0 * EPS;
         cache[threadIdx.x] = cache[threadIdx.x] * -1.0;

         FUNCTION(N, &space[i * N], &cache[threadIdx.x]);
         grad[i] = cache[threadIdx.x] / (2.0 * EPS);
      }
   };
   ...
   inline void linalg_grad( long int N ,  double EPS, vector<double>& x,  vector<double>& grad, double* _space) {
      ...
      bulkGrad  <<< GPU_BLOCK_1D , GPU_TPB_1D , 6000 / GPU_BLOCK_1D.x >>> (N, EPS, _space, _grad);
